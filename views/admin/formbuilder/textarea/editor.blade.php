<div class="uk-form-row form-group-{{ $row_key }} {{ $row_settings->css_class_group }}">
    ...
    <textarea name="{{ $row_key }}" class="uk-textarea form-control {{ $row_settings->css_class }}" id="{{ $row_key }}">@isset($data->{$row_key}){{ Request::old($row_key, $data->{$row_key}) }}@endisset</textarea>
</div>